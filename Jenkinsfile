pipeline {
    ...
    stages {
        stage('Build') {
            steps {
               sh '''
                docker build -t satishgssk/task1-python-img .
                '''
            }
        }
        stage('Push') {
            steps {
               sh '''
                docker push satishgssk/task1-python-img
                '''
            }
        }
        stage('Deploy') {
            steps { 
                ...
                docker run -d --name flask-app-sat --network nginx-net satishgssk/task1-python-img
                '''
            }
        }
    }
